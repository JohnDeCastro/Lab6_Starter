...
class RecipeCard extends HTMLElement {
	...
	constructor() {
		...
		this.shadow = this.attachShadow({mode: 'open'});//mode left open, creates encapsulate DOM or shadowDOM
		// A2. TODO - Create an <article> element - This will hold our markup once our data is set
		const article = document.createElement('article');//holds card contents
		// A3. TODO - Create a style element - This will hold all of the styles for the Web Component
		const style = document.createElement('style');//inject CSS from html. only appears in shadowDOM
		...
		style.textContent = `
    		* { font-family: sans-serif; margin: 0; padding: 0; }
    		...
    		article { align-items: center; border: 1px solid rgb(223, 225, 229); border-radius: 8px; display: grid; grid-template-rows: 118px 56px 14px 18px 15px 36px; height: auto; row-gap: 5px; padding: 0 16px 16px 16px; width: 178px; }
  		...
  		`;//everything from <style>
		// A5. TODO - Append the <style> and <article> elements to the Shadow DOM
		this.shadow.appendChild(style);//appends style element into shadowDOM
		this.shadow.appendChild(article);//appends article alament into shadowDOM
	}
	...
	set data(data) {
		...
		const article = this.shadow.querySelector('article');//grab empty article to populate
